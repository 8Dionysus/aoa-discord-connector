...
This card applies to `aoa-discord-connector/kag/` and every nested path until a
nearer card narrows the lane.

## Role

`kag/` is the local KAG provider home for `aoa-discord-connector`.

It exposes compact source-linked records over the Discord connector policy,
