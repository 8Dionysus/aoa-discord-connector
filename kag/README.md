# aoa-discord-connector Local KAG Provider

`kag/` exposes the current Discord connector provider packet as portable
...
| Field | Route |
| --- | --- |
| role | local KAG provider for Discord connector policy, storage boundary, and runtime contract handles |
...
| edge | source surface routes to the permission and runtime boundary |
