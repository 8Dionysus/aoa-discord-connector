#!/usr/bin/env python3
"""Validate the GitHub-publishable Discord connector skeleton."""
...
COMMAND_LINE_RE = re.compile(
    ...
    r"aoa-discord(?:\s|$)|pip(?:3)?\s+|uv\s+|git\s+|export\s+)",
    re.IGNORECASE,
)
...
REQUIRED_FILES = [
    ...
    "connector/fixtures/discord/starter_conversation.json",
    ...
    "docs/decisions/AOA-DISCORD-D-0001-permissioned-conversation-proof.md",
    "evals/AGENTS.md",
    "evals/PORT.yaml",
    ...
    "stats/packets/starter-guild-message-content-observability-ratio.bot-gateway.reference.json",
    "stats/packets/starter-guild-message-content-observability-ratio.message-content.reference.json",
    "src/aoa_discord_connector/cli.py",
]
...
REQUIRED_DIRS = [
    ...
    "src/aoa_discord_connector/parse",
    "src/aoa_discord_connector/normalize",
    "src/aoa_discord_connector/index",
    "src/aoa_discord_connector/graph",
    "src/aoa_discord_connector/claims",
    "src/aoa_discord_connector/answer",
    "src/aoa_discord_connector/query",
    "src/aoa_discord_connector/evaluation",
    "src/aoa_discord_connector/storage",
    "tests/unit",
    "tests/contract",
    "tests/integration",
    "evals/intake",
...
def main() -> int:
    ...
    payload = {
        "schema": "aoa_discord_connector_validation_v1",
...
def _check_text(repo_root: Path, errors: list[str], warnings: list[str]) -> None:
    ...
    for token in ["gateway", "message content", "privileged intent", "rest history", "data package", "selfbot", "user-token", "attachments", "download", "write", "internal search"]:
        if token not in source_policy.casefold():
            errors.append(f"source policy missing denied token: {token}")

    for token in ["bot_gateway", "bot_gateway_message_content", "rest_history", "data_package", "user_token_selfbot", "download", "write", "internal-search"]:
...
def _check_eval_port(repo_root: Path, errors: list[str]) -> None:
    ...
    for token in [
        ...
        "owner_repo: aoa-discord-connector",
    ...
    expected_suites = {
        "starter_claim_conflict_relations.json": "aoa_discord_permission_eval_suite_v1",
        "starter_claim_answer_packets.json": "aoa_discord_answer_eval_suite_v1",
    }
