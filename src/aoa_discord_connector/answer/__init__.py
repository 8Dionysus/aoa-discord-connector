"""Render permission-aware answer packets from Discord evidence packets."""
...
def render_answer_packet(evidence_packet: dict[str, object], limit: int = 5) -> dict[str, object]:
    ...
    return {
        ...
        "answer_report": {
            "renderer": "discord_permissioned_conversation_answer_v1",
...
def _answer_item(result: dict[str, object]) -> dict[str, object]:
    return {
        "answer_kind": "discord_message_evidence",
...
def _agent_answer(status: str, evidence_chain: list[dict[str, object]]) -> str:
    if status == "answered":
        ...
        return f"Local Discord evidence supports an answer from {first.get('conversation_id')} message {first.get('message_id')}."
    if status == "insufficient_permission":
        return "The local Discord connector lacks permission for the requested scope."
    return "The local Discord connector does not have enough local evidence for this question."
