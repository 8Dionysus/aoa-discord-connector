"""Command line interface for the Discord conversation connector."""
...
from aoa_discord_connector.answer import render_answer_packet
from aoa_discord_connector.config import StorageRoots, find_repo_root
from aoa_discord_connector.graph import build_graph
from aoa_discord_connector.index import build_keyword_index
from aoa_discord_connector.normalize import DEFAULT_MODE, SUPPORTED_MODES, normalize_snapshot
from aoa_discord_connector.policy.rules import route_decision
from aoa_discord_connector.query import query_graph_packet, query_keyword_index
from aoa_discord_connector.sources import (
    ACCESS_MODES,
    MEDIA_POLICIES,
    SOURCE_KINDS,
    build_source_plan,
...
from aoa_discord_connector.storage import create_storage_roots, storage_status
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="aoa-discord")
    ...
    sources_plan.add_argument("--run", default="discord-owned-sources")
...
def cmd_init(_args: argparse.Namespace) -> int:
    ...
    _emit({"schema": "aoa_discord_init_v1", "status": "ok", "created": create_storage_roots(roots), "network_touched": False})
    return 0
...
def cmd_sources_add(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        _emit({"schema": "aoa_discord_source_registry_receipt_v1", "status": "error", "error": str(exc), "network_touched": False, "read_only": True})
        return 2
    _emit({"schema": "aoa_discord_source_registry_receipt_v1", "status": "ok", "state": state, "registry_path": str(path), "source": source, "network_touched": False, "read_only": True})
    return 0
...
def cmd_sources_list(args: argparse.Namespace) -> int:
    ...
    _emit({"schema": "aoa_discord_source_registry_list_v1", "status": "ok", "registry_path": str(registry_path(roots.data)), "source_count": len(registry.get("sources", [])), "selected_count": len(selected), "sources": selected, "network_touched": False, "read_only": True})
    return 0
...
def cmd_policy_check(_args: argparse.Namespace) -> int:
    samples = [
        "discord:bot_gateway:guild:aoa-lab:channel:android",
        "discord:rest_history:guild:aoa-lab:channel:android",
        "discord:data_package:account:self",
        "discord:user_token_selfbot:dm:self",
        "discord:write:send_message",
    ...
    _emit({"schema": "aoa_discord_policy_check_v1", "status": "ok" if ok else "error", "decisions": decisions, "network_touched": False})
...
def cmd_materialize_fixture(args: argparse.Namespace) -> int:
    ...
    normalized_path = normalize_snapshot(raw_path, "discord:fixture", normalized_dir, mode=args.mode)
    receipt = {
        "schema": "aoa_discord_materialize_receipt_v1",
        "run_id": args.run,
...
def cmd_build_index(args: argparse.Namespace) -> int:
    ...
    _emit({"schema": "aoa_discord_index_receipt_v1", "status": "ok", "run_id": args.run, "index_path": str(path), "doc_count": index["doc_count"], "term_count": index["term_count"], "network_touched": False})
    return 0
...
def cmd_build_graph(args: argparse.Namespace) -> int:
    ...
    _emit({"schema": "aoa_discord_graph_receipt_v1", "status": "ok", "run_id": args.run, "graph_path": str(path), "node_count": graph["node_count"], "edge_count": graph["edge_count"], "network_touched": False})
    return 0
...
def cmd_eval_permissions(_args: argparse.Namespace) -> int:
    gateway = _proof("bot_gateway")
    content = _proof("bot_gateway_message_content")
    data_package = _proof("data_package")
    ok = gateway["permission_gap_count"] >= 1 and content["permission_gap_count"] == 0 and data_package["data_package_dm_visible"] is True
    _emit({"schema": "aoa_discord_permission_eval_v1", "status": "pass" if ok else "fail", "bot_gateway": gateway, "bot_gateway_message_content": content, "data_package": data_package, "network_touched": False})
    return 0 if ok else 1


def cmd_eval_answer_packets(_args: argparse.Namespace) -> int:
    run = "eval-answer-fixture"
    roots = _materialize_build(run, "bot_gateway_message_content")
    ...
    _emit({"schema": "aoa_discord_answer_eval_v1", "status": "pass" if ok else "fail", "cases": [warning["answer_report"], missing["answer_report"]], "network_touched": False})
    return 0 if ok else 1
...
def _proof(mode: str) -> dict[str, object]:
    ...
    return {
        ...
        "permission_gap_count": len(index.get("permission_gaps", [])),
        "data_package_dm_visible": any(doc.get("conversation_type") == "data_package_dm" for doc in index.get("docs", [])),
    }
...
def _materialize_build(run: str, mode: str, build_graph_too: bool = True) -> StorageRoots:
    ...
    normalize_snapshot(raw_path, "discord:fixture", normalized_dir, mode=mode)
