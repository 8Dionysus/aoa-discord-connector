"""Normalize authorized Discord fixture snapshots into conversations."""
...
SUPPORTED_MODES = {"bot_gateway", "bot_gateway_message_content", "rest_history", "data_package"}
DEFAULT_MODE = "bot_gateway"


def normalize_snapshot(raw_path: Path, _source_url: str, output_dir: Path, mode: str = DEFAULT_MODE) -> Path:
    if mode not in SUPPORTED_MODES:
        raise ValueError(f"unsupported Discord ingest mode: {mode}")
    ...
    for conversation in snapshot.get("conversations", []):
        ...
        conversations.append(
            {
                "schema": "aoa_discord_normalized_conversation_v1",
                "source": "discord",
    ...
    output_path = output_dir / f"discord-conversations-{mode}.json"
    output_path.write_text(json.dumps({"schema": "aoa_discord_normalized_snapshot_v1", "conversations": conversations}, indent=2), encoding="utf-8")
    return output_path


def _normalize_message(message: dict[str, object], conversation: dict[str, object], mode: str, captured_at: str) -> dict[str, object]:
    text_available = _text_available(message, mode)
    text = str(message.get("text") or "") if text_available else ""
    return {
        "schema": "aoa_discord_normalized_message_v1",
        "source": "discord",
        "source_mode": mode,
        "conversation_id": conversation["conversation_id"],
        "conversation_type": conversation["conversation_type"],
        ...
        "entities": _extract_entities(text, message) if text_available else [],
...
def _permission_state(message: dict[str, object], mode: str) -> dict[str, object]:
    if not _text_available(message, mode):
        return {
            "status": "insufficient_permission",
            "mode": mode,
            "requires_message_content_intent": True,
            "reason": "message_content_intent_missing",
        }
    private = message.get("sensitivity") in {"private_dm", "closed_group", "account_data_package"}
    return {
        "status": "authorized",
        "mode": mode,
        "requires_message_content_intent": bool(message.get("requires_message_content_intent", False)),
        "requires_operator_local_session": private,
        "reason": "account_owned_data_package" if mode == "data_package" else "bot_scope_and_channel_permissions",
    }


def _text_available(message: dict[str, object], mode: str) -> bool:
    if message.get("requires_message_content_intent") and mode == "bot_gateway":
        return False
    return True
...
def _extract_entities(text: str, message: dict[str, object]) -> list[dict[str, str]]:
    ...
    for token in ["gateway", "message content", "rest history", "data package", "xiaomi 13t", "hyperos", "vendor_boot.img", "boot.img", "magisk", "fastboot"]:
        if token in lowered:
