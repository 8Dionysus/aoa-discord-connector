"""Build a small conversation graph from normalized Discord messages."""
...
def build_graph(normalized_dir: Path, output_dir: Path, profile_id: str = "starter-permissioned") -> Path:
    ...
    for snapshot_path in sorted(normalized_dir.glob("discord-conversations-*.json")):
        ...
        for conversation in snapshot.get("conversations", []):
            ...
            for message in conversation.get("messages", []):
                ...
                nodes[message_node] = _node(message_node, "message", f"Discord message {message['message_id']}", message.get("source_url"))
    ...
    payload = {
        "schema": "aoa_discord_conversation_graph_v1",
...
def _node(node_id: str, kind: str, label: object, source_ref: object) -> dict[str, object]:
    return {"schema": "aoa_discord_graph_node_v1", "node_id": node_id, "kind": kind, "label": str(label or node_id), "source_refs": [str(source_ref or "")]}
...
def _edge(edges: list[dict[str, object]], kind: str, from_node: str, to_node: str, source_ref: object, confidence: float) -> None:
    ...
    edges.append(
        {
            "schema": "aoa_discord_graph_edge_v1",
            "edge_id": edge_id,
            "kind": kind,
            "from_node": from_node,
            "to_node": to_node,
