"""Operator-local Discord source registry and sync planning."""
...
SOURCE_KINDS = {"guild_channel", "thread", "dm", "data_package"}
ACCESS_MODES = {"bot_visible", "message_content", "private_authorized", "account_owned_export"}
MEDIA_POLICIES = {"none", "thumbnails", "documents", "all"}
REGISTRY_SCHEMA = "aoa_discord_source_registry_v1"
SOURCE_SCHEMA = "aoa_discord_source_v1"
PLAN_SCHEMA = "aoa_discord_source_sync_plan_v1"


def registry_path(data_root: Path) -> Path:
    return data_root / "sources" / "discord_sources.json"
...
def load_registry(data_root: Path) -> dict[str, object]:
    ...
    if registry.get("schema") != REGISTRY_SCHEMA:
        raise ValueError(f"unsupported Discord source registry schema: {registry.get('schema')}")
    return registry
...
def upsert_source(
    ...
    *,
    ...
    kind = kind.strip()
    access = access or _default_access(kind)
    ...
    source = {
        ...
        "route": f"discord:{kind}:{source_ref}",
        "kind": kind,
        "access": access,
...
def _validate(source_ref: str, kind: str, access: str, include_media: str, trust_score: float | None) -> None:
    ...
    if kind not in SOURCE_KINDS:
        raise ValueError(f"unsupported Discord source kind: {kind}")
    if access not in ACCESS_MODES:
        raise ValueError(f"unsupported Discord access mode: {access}")
...
def _default_access(kind: str) -> str:
    if kind == "data_package":
        return "account_owned_export"
    if kind == "dm":
...
def _source_id(kind: str, source_ref: str) -> str:
    ...
    return f"discord-src-{digest[:16]}"
