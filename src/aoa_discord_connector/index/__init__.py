"""Small deterministic keyword index for normalized Discord messages."""
...
def build_keyword_index(normalized_dir: Path, output_dir: Path, profile_id: str = "starter-permissioned") -> Path:
    ...
    for snapshot_path in sorted(normalized_dir.glob("discord-conversations-*.json")):
        ...
        for conversation in snapshot.get("conversations", []):
            ...
            for message in conversation.get("messages", []):
                ...
                if isinstance(permission_state, dict) and permission_state.get("status") != "authorized":
    ...
    payload = {
        "schema": "aoa_discord_keyword_index_v1",
        "profile_id": profile_id,
        "unit": "discord_message",
